...
class guiThread (threading.Thread): #Just going to run in the main thread
  ...
  while 1:
    ...
    for sock in ready_for_reading:  
      ...
      if sock == server_socket:
        ...
        data = sock.recv(4096)
        if not data:
          #print '\nDisconnected from chat server' # Maybe add this as an error to be caught by the system if the user tries to send a message after they've been disconected
          sys.exit()
        else:
    ...
    if sending == 1:
      ...
      #create_window()
        ...
        #message_date =  # either an array with each item or, figure out how to send the username, cityname, messagge
        #server_socket.send(messageg_data)
      sending = 0;
