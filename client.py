...
def founded_city(create_list_button): #possibly consider making a function for disabling buttons to reduce code copyingg
    create_list_button.enabled = 0
    join_list_button.enabled = 0
    join_button.enabled = 0
    create_button.enabled = 0
    current_city = free_cities[create_list_button.value + 1]
...
def joined_city(join_list_button):
    join_list_button.enabled = 0
    create_list_button.enabled = 0
    join_button.enabled = 0
    create_button.enabled = 0
...
join_list_button = ListButton(position = (30, join_button.bottom + 30), 
    ...
    action = joined_city(join_list_button))
...
create_list_button = ListButton(position = (join_list_button.right + 30, create_button.bottom + 30), 
    ...
    action = founded_city(create_list_button))
...
def refresh_buttons():
    join_button_v2 = Button(position = (30, 30), 
        title = "Join City", 
        action = occupied_city_list, 
        style = 'cancel')

    join_list_button_v2 = ListButton(position = (30, join_button.bottom + 30), 
        titles = ["Move to %s" % taken_cities[i] for i in taken_cities], #need to add so only occupied free_cities are avaliable to be moved to
        action = joined_city(join_list_button_v2))

    # create_button for creating a city

    create_button_v2 = Button(position = (join_list_button.right + 30, 30),
        title = "Create City", 
        action = empty_city_list, 
        style = 'cancel')
    ...
    create_list_button_v2 = ListButton(position = (join_list_button.right + 30, create_button.bottom + 30), 
        titles = ["Found %s" % free_cities[i] for i in free_cities], #need to add so only occupied free_cities are avaliable to be moved to
        action = founded_city(create_list_button_v2))

    leave_button_v2 = Button(position = (create_list_button.right + 30, 30), 
        title = "Leave City", 
        action = leave_city, 
        style = 'cancel')   

    refresh_window(join_button_v2, join_list_button_v2, create_button_v2, create_list_button_v2, leave_button_v2) 
...
def refresh_window(btn1, btn2,btn3,btn4,btn5):
    window.add(btn1)
    window.add(btn2)
    window.add(btn3)
    window.add(btn4)
