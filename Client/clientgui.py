# CHATCITY CLIENT 
# Authors: Juan Comish, Rhett Devlin, Austin Lee
# Date: August 2nd 2017
# Version: 1.0
# Description: This application is the implementation of a messaging system that implements chat rooms, aliases, and a GUI. This file is the client side and will require the server is running.
...
if(len(sys.argv) < 2): #For declaring the ip for connecting 
...
def chat_client(host,port):
  ...
  while 1: #Scanning for input/output
    ...
    for sock in ready_for_reading:  
      ...
      if sock == server_socket: #incoming message from remote server, server_socket
        ...
        if not data:
          ...
          tempdata = data # scan through tempdata to pick out the cityname, alias, and message
          ...
          refresh_output_receive()#refreshes the output terminal on the GUI
